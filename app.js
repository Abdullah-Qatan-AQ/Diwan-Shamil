...
async function loadLexicon() {
  ...
  try {
    const poetryOriginal = decode(await fetchJson("./data/lexicon/poetry-original-glosses.json", 12000));
    for (const [key, values] of Object.entries(poetryOriginal.entries)) {
      state.lexicon[key] = [...values, ...(state.lexicon[key] || [])];
    }
  } catch {
    // ملف المعاني الشعرية الإضافي اختياري عند تشغيل نسخة قديمة من الحزمة.
  }
...
async function downloadLibrary() {
  ...
  const urls = [
    ...
    "./data/lexicon/poetry-original-glosses.json",
