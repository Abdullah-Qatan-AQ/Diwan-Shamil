  ...
  $("#poems").innerHTML =
    visible
      .map((item) => {
        ...
        const attribution = item.attribution ? `<small class="poem-attribution">${esc(item.attribution)}${item.source_url ? ` · <a href="${esc(item.source_url)}" target="_blank" rel="noopener">المصدر</a>` : ""}</small>` : "";
        return `<article class="poem" data-entry-id="${esc(id)}"><header><b>${esc(item.poem_title || "قصيدة")}</b><span>${esc(item.poet_name || "شاعر")} · ${esc(item.poet_era || "")} <span class="entry-actions"><button type="button" class="mini-save" data-save="${esc(id)}" data-title="${esc(item.poem_title || "قصيدة")}">${state.bookmarks.some((saved) => saved.id === id) ? "★" : "☆"}</button><button type="button" class="mini-copy" data-copy="${esc(poemText)}" aria-label="نسخ القصيدة">نسخ النص</button><button type="button" class="mini-position" data-save-position='${esc(JSON.stringify(position))}' aria-label="حفظ موضع القصيدة">⌖</button></span></span></header><p>${esc(poemText)}</p>${attribution}</article>`;
      })
